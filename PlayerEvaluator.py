...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog='PlayerEvaluator', description="Avalua jugadors del joc d'EDA. Executa N partides del joc amb seeds consecutives i els jugadors que s'hagin definit, i mostra un resum de les principals mètriques resultants", epilog='')
    parser.add_argument('jugadors', metavar='jugador', type=str, nargs=4, default=[
                        'Dummy', 'Dummy', 'Dummy', 'Dummy'], help="Noms dels 4 jugadors que s'executaran. Cal que estiguin definits al joc")
    parser.add_argument('-c', '--cnf', default='default.cnf',
                        help="Arxiu de configuració que volem que utilitzi el joc. Cal que pengi del directori game")
    parser.add_argument('-d', '--desa-partides', action=argparse.BooleanOptionalAction,
                        help='Conserva la partida sencera (arxiu .res)')
    parser.add_argument('-s', '--seed-min', default=None, type=int,
                        help="Seed mínima. Si no s'indica, serà aleatòria")
    parser.add_argument('-n', '--num-proves', default=100,
                        type=int, help="Nombre d'iteracions a executar")
    ...
    for i in range(args.seed_min, args.seed_min+args.num_proves):
        ...
        with open(nom_resultat, 'w') as f:
            ...
            print(
                f'Resultat de la partida desat a {nom_resultat}', file=sys.stderr)
